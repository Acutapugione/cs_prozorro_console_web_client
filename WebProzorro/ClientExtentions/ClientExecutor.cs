...
public class ClientExecutor
{
    ...
    public async Task<HashSet<BaseItemDTO>> LoadIndexesByType(string typeName = "", int count = 0)
    {
        ...
        var startPoint = await GetBaseContainerDTOAsync<BaseContainerDTO<BaseItemDTO>>($"/api/{typeName}");
        ...
        do
        {
            ...
            startPoint = await GetBaseContainerDTOAsync<BaseContainerDTO<BaseItemDTO>>(path);
    ...
    public async Task<HashSet<T>> LoadObjectsByType<T>(HashSet<BaseItemDTO> containers, string typeName = "", int count = 0)
    {
        ...
        foreach (var item in containers)
        {
            ...
            var offer = await GetBaseContainerDTOAsync<T>($"/api/{typeName}/{item.Id}", "data");
    ...
    public async Task<HashSet<T>> LoadItems<T>(string typeName = "products")
    {
        try
        {
            HashSet<BaseItemDTO> indexes = await
                LoadIndexesByType(
                typeName: typeName,
                count: 100);

            HashSet<T> items = await
                LoadObjectsByType<T>(
                indexes,
                typeName: typeName,
                count: 100);
